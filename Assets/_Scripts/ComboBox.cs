...
{
	public static ComboBox SELECTED_BOX;
	...
	{
		GUI.enabled = (SELECTED_BOX == null || SELECTED_BOX == this);
	    ...
	    if( isClickedComboButton && GUI.enabled )
	    {
			float ySize = listStyle.CalcHeight(listContent[0], 1.0f);
			
	        Rect listRect = new Rect( rect.x, rect.y + ySize, rect.width, ySize * listContent.Length );
			
			if (rect.y + ySize + ySize * listContent.Length > Screen.height)
			{
				listRect = new Rect( rect.x, rect.y - ySize * listContent.Length, rect.width, ySize * listContent.Length );
			}
	...
	public static void UnSelect()
	{
		...
		{
			SELECTED_BOX.isClickedComboButton = false;
			SELECTED_BOX = null;
		}
	}
}